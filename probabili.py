...
def _resolve_folder_id(url_or_id: str) -> str:
    """Accetta URL di cartella Drive o ID e ritorna l'ID pulito."""
    if not url_or_id:
        return ""
    s = url_or_id.strip()
...
def _find_by_name_in_folder(drive, folder_id: str, filename: str) -> str | None:
    ...
    (Niente f-string: costruiamo la query con .format)
    """
...
def main():
    # Pattern dei PNG da caricare (arriva dal workflow)
    image_glob = os.environ.get("IMAGE_GLOB", "output/**/*.png").strip()
    print("🔎 IMAGE_GLOB: {}".format(image_glob))

    # URL/ID cartella (secret/var DRIVE_FOLDER_URL)
    ...
    try:
        ...
        print("❌ Errore sul target della cartella: {}".format(e))
        return

    for p in files:
        if not os.path.isfile(p):
            print("⚠️  Salto (non esiste): {}".format(p))
            continue
